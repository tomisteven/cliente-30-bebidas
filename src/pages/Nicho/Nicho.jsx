...
const Nicho = () => {
    ...
    const fetchInitialProducts = useCallback(async () => {
        ...
        setPage(1);
        try {
            const data = await getProducts({ search, categoria: 'Nicho', sort, limit: 20, page: 1 });
            setProducts(data.products || []);
            setHasMore(data.products?.length === 20);
        } catch (error) {
            console.error('Error fetching nicho products:', error);
        } finally {
            setLoading(false);
        }
    }, [search, sort]);
    ...
    const loadMoreProducts = async () => {
        ...
        try {
            const data = await getProducts({ search, categoria: 'Nicho', sort, limit: 20, page: nextPage });
            ...
            console.error('Error loading more nicho products:', error);
        } finally {
        ...
        <div className={styles.page}>
                ...
                <div className={styles.controlsContainer}>
                    <div className={styles.searchWrapper}>
                        <div className={styles.searchBar}>
                            ...
                            <input
                                type="text"
                                placeholder="Buscar en la colección Nicho..."
                                value={search}
            ...
            <div className="container">
                <main className={styles.mainContent}>
                    {loading ? (
                        <div className={styles.loader}>Descubriendo exclusividades...</div>
                    ) : (
                        <div className={viewMode === 'grid' ? styles.grid : styles.list}>
                            {products.length > 0 ? (
                                ...
                                <div className={styles.noResults}>
                                    <p>No se encontraron perfumes nicho en este momento.</p>
                                </div>
                            )}
                        </div>
                    )}
